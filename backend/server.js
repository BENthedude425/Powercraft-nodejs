...
async function JWTCheck (token) {
  ...
  return new Promise((resolve, reject) => [
    DATABASECONNECTION.query(SQLquery, [token], (error, results, fields) => {
      if (error) {
        reject()
      }
...
async function GenerateAuthToken () {
  ...
  while (!found) {
    ...
    await new Promise((resolve, reject) => {
      DATABASECONNECTION.query(SQLquery, [authToken], (error, results, _) => {
        if (error) {
          modules.Log(FILEIDENT, error)
        }
...
function GetUniqueServerID () {
  return new Promise((resolve, reject) => {
    DATABASECONNECTION.query('SELECT * FROM servers', (error, results, _) => {
      if (error != null) {
        modules.Log(error)
        return
      }
...
app.post('/api/login', async (req, res) => {
  const userExists = await CheckUserExists(req.body.username, req.body.password)
...
function FillServerPropertiesOptions (serverProperties) {
  ...
  console.log(newServerProperties)
...
if (DEVMODE) {
  app.get('/db/*', async (req, res) => {
    ...
    DATABASECONNECTION.query(`SELECT * FROM ${dbName}`, (error, results, _) => {
      if (error != null) {
        modules.Log(FILEIDENT, error)
